...
export function createAccountRemoveHandler(db: DataSession) {
  return {
    async preview(accountIds: number[]): Promise<Result<AccountRemovePreview, Error>> {
      ...
      const [projectionImpactResult, costBasisResult, purgeResult] = await Promise.all([
        ...
        buildCostBasisResetPorts(db).countResetImpact(),
        buildIngestionPurgePorts(db).countPurgeImpact(accountIds),
      ]);
    ...
    async execute(accountIds: number[]): Promise<Result<AccountRemoveResult, Error>> {
      ...
      const removalResult = await db.executeInTransaction(async (txDb) => {
