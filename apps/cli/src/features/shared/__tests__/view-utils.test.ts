...
import { buildViewMeta, getAllMovements, parseDate } from '../../../cli/view-utils.js';

describe('parseDate', () => {
  it('should parse valid ISO date string', () => {
    const result = parseDate('2024-01-01');
    const date = assertOk(result);
  ...
  it('should throw error for invalid date string', () => {
    ...
    expect(assertErr(result).message).toBe('Invalid date format: invalid-date');
  });

  it('should throw error for empty string', () => {
    const result = parseDate('');
    expect(assertErr(result).message).toBe('Invalid date format: ');
  });

  it('should throw error for invalid month', () => {
    const result = parseDate('2024-13-01');
    expect(assertErr(result).message).toBe('Invalid date format: 2024-13-01');
  });

  it('should throw error for invalid day', () => {
    const result = parseDate('2024-01-32');
    expect(assertErr(result).message).toBe('Invalid date format: 2024-01-32');
  });

  it('should throw error for malformed string', () => {
    const result = parseDate('not a date');
    expect(assertErr(result).message).toBe('Invalid date format: not a date');
  });
...
describe('getAllMovements', () => {
  it('should return empty array when no movements', () => {
    const movements: TransactionDraft['movements'] = {
      inflows: [],
      outflows: [],
    };
