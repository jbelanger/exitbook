import {
  buildLinkGapCrossProfileCounterpartsByIssueKey,
  buildLinkGapIssueKey,
  buildVisibleProfileLinkGapAnalysis,
  type LinkGapAnalysis,
  type LinkGapCrossProfileCounterpart,
...
export async function buildLinksGapsBrowsePresentation(
  ...
  return resultDoAsync(async function* () {
    ...
    const crossProfileCandidatesByIssueKey = materializeCrossProfileCandidateRefs(
      buildLinkGapCrossProfileCounterpartsByIssueKey(sortedAnalysis.issues, source.crossProfileContext)
    );
...
function materializeCrossProfileCandidateRefs(
  counterpartsByIssueKey: ReadonlyMap<string, readonly LinkGapCrossProfileCounterpart[]>
): Map<string, LinkGapBrowseCrossProfileCandidate[]> {
  return new Map(
    [...counterpartsByIssueKey.entries()].map(([issueKey, counterparts]) => [
      issueKey,
      counterparts.map((counterpart) => ({
        ...counterpart,
        transactionRef: formatTransactionFingerprintRef(counterpart.txFingerprint),
      })),
    ])
  );
}
