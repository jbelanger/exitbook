...
export function pricesEnrichReducer(state: PricesEnrichState, action: PricesEnrichAction): PricesEnrichState {
  switch (action.type) {
    ...
    case 'complete':
      ...
      return { ...state, isComplete: true, totalDurationMs: performance.now() - state.startedAt };
    case 'abort':
      if (state.isComplete) return state;
      return { ...state, isComplete: true, aborted: true, totalDurationMs: performance.now() - state.startedAt };
    case 'fail':
      if (state.isComplete) return state;
      return {
        ...state,
