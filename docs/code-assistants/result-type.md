...
## Composition with `resultDo` (sync)
...
## Composition with `resultDoAsync` (async)
...
`resultTry` / `resultTryAsync` — same as above but catches thrown exceptions and wraps them in Err. Use when calling throwing APIs (database, network):
...
- Use `resultDo` / `resultDoAsync` for composition — not `.andThen()` chaining
- Do NOT use `resultDo` for `AsyncIterableIterator` generators that yield multiple Results over time
