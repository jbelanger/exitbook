...
| `links`        | `links run`, `links view`, `links view --gaps`, `links gaps` (compat), `confirm`, `reject` | likely `links`, `links list`, `links view <fingerprint>`, `links explore [fingerprint]`, plus review/workflow commands            | stable          | `in_progress` | Phase 0 is clarifying `status` vs `gaps` semantics before the full browse-ladder migration.                       |
...
- `links` phase 0 is semantic cleanup first: `status` remains `suggested|confirmed|rejected`; `gaps` is a separate analysis lens; `needs-review` is deferred until a unified queue exists.
