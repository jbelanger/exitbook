...
- **Separation of Concerns:** The process of _fetching_ raw data must be completely decoupled from the process of _transforming_ it into a canonical format.
...
```
 ...
 └────────────┬───────────────────┬──────┘
              │ 1. importFromSource()   │ 2. processRawDataToTransactions()
              ▼                         ▼
  +---------------------------+     +-------------------------------+
...
- **Function:** It validates input parameters (like a Bitcoin address) and then calls the `BlockchainProviderManager` to execute the necessary data fetching operations. It does _not_ contain any logic for communicating with specific APIs.
...
This separation is key: the `Processor` knows the _business rules_ of Bitcoin, while the `Mapper` knows the _data structure_ of the Mempool.space API.
