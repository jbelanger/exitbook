...
import type {
  ...
  XpubType
} from '../core/types/bitcoin';
import { Logger } from '../infrastructure/logging';
import { BlockchainProviderManager } from '../providers/shared/BlockchainProviderManager.js';

const logger = new Logger('BitcoinUtils');
    ...
    for (let i = 0; i < allDerived.length; i++) {
      ...
      try {
        const addressInfo = await providerManager.executeWithFailover('bitcoin', {
          type: 'getAddressInfo',
          params: { address },
          getCacheKey: (params) => `bitcoin:address-info:${params.address}`
        }) as AddressInfo;
