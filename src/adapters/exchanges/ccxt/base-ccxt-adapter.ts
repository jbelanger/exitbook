...
import { ServiceErrorHandler } from '../../../utils/exchange-error-handler';
import { TransactionTransformer } from '../../../utils/transaction-transformer';
...
export abstract class BaseCCXTAdapter implements IExchangeAdapter {
  ...
  constructor(exchange: Exchange, config: ExchangeConfig, enableOnlineVerification: boolean = false, loggerSuffix?: string) {
    ...
    this.logger = new Logger(`${loggerSuffix || 'CCXTAdapter'}:${config.id}`);
