...
// Enhanced types for better parsing
export interface ParsedInjectiveTransaction {
  hash: string;
  blockNumber: number;
  blockHash: string;
  timestamp: number;
  ...
  type: 'transfer' | 'contract_execution' | 'token_transfer';
  tokenSymbol: string;
  confirmations: number;
  messages: InjectiveMessage[];
  rawTransaction: InjectiveTransaction;
}