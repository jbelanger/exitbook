...
describe('Universal Schemas Validation', () => {
  describe('MoneySchema', () => {
    ...
    it('should reject money with invalid amount type', () => {
      ...
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('Expected Decimal instance');
      }
    });
    ...
    it('should reject money with empty currency', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('Currency must not be empty');
      }
    });
  });
  ...
  describe('UniversalBalanceSchema', () => {
    ...
    it('should reject balance where total < free + used', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('Total balance must be >= free + used');
      }
    });
  ...
  describe('Validation Helper Functions', () => {
    ...
    describe('validateUniversalBalances (batch)', () => {
      it('should separate valid and invalid balances', () => {
        ...
        const result = validateUniversalBalances([validBalance, invalidBalance]);
