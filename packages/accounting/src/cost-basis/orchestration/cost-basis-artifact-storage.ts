...
import type { AcquisitionLot, CostBasisCalculation, LotDisposal, LotTransfer } from '../shared/schemas.js';

import type { CostBasisWorkflowResult } from './cost-basis-workflow.js';

const logger = getLogger('cost-basis-artifact-storage');
...
export const StoredCostBasisArtifactEnvelopeSchema = z.discriminatedUnion('artifactKind', [
  StoredArtifactEnvelopeBaseSchema.extend({
    artifactKind: z.literal('generic'),
    artifact: StoredGenericCostBasisArtifactSchema,
    debug: StoredGenericDebugSchema,
  }),
  StoredArtifactEnvelopeBaseSchema.extend({
...
type StoredGenericArtifact = z.infer<typeof StoredGenericCostBasisArtifactSchema>;
...
export interface CostBasisArtifactDebugPayload {
  kind: 'generic-pipeline' | 'canada-workflow';
...
export function readCostBasisSnapshotArtifact(
  ...
  const artifact =
    envelope.artifactKind === 'generic'
      ? fromStoredGenericArtifact(envelope.artifact)
...
function fromStoredGenericArtifact(
  artifact: StoredGenericArtifact
): Extract<CostBasisWorkflowResult, { kind: 'generic-pipeline' }> {
...
function toStoredGenericDebug(debug: CostBasisArtifactDebugPayload): z.infer<typeof StoredGenericDebugSchema> {
  return {
    kind: 'generic-pipeline',
...
function fromStoredDebug(debug: StoredCostBasisDebug): CostBasisArtifactDebugPayload {
  return debug.kind === 'generic-pipeline'
    ? {
        kind: debug.kind,
        scopedTransactionIds: debug.scopedTransactionIds,
        appliedConfirmedLinkIds: debug.appliedConfirmedLinkIds,
