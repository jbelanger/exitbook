...
export class CostBasisReportGenerator {
  ...
  async generateReport(config: CostBasisReportInput): Promise<Result<CostBasisReport, Error>> {
    ...
    try {
      ...
      const jurisdictionRulesResult = resolveCostBasisJurisdictionRules(calculation.config.jurisdiction);
      if (jurisdictionRulesResult.isErr()) {
        return err(jurisdictionRulesResult.error);
      }

      // Calculate summary totals in display currency
      const summary = this.calculateSummary(convertedDisposals, jurisdictionRulesResult.value);
