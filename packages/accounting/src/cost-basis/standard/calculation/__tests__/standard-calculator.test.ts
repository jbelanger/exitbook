...
import { calculateScopedCostBasis } from '../standard-calculator.js';
...
describe('calculateScopedCostBasis', () => {
  ...
  describe('calculate', () => {
    ...
    it('should reject Canada in the standard calculator', async () => {
      ...
      expect(resultError.message).toContain('specialized Canada workflow');
    });
    ...
    it('should reject average-cost in the standard calculator', async () => {
      ...
      const resultError = assertErr(result);
      expect(resultError.message).toContain('Canada workflow');
    });
