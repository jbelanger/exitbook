...
/**
 * Service for inferring transaction movement prices
...
export class PriceInferenceService {
  ...
  async derivePrices(): Promise<Result<{ transactionsUpdated: number }, Error>> {
    try {
      logger.info('Starting price inference process');
      ...
      logger.info({ transactionsUpdated: updatedCount }, 'Price inference completed');
