...
import type { StoredRawData } from '../types';
...
export class RawDataRepository implements IRawDataRepository {
  ...
  async load(filters?: LoadRawDataFilters): Promise<StoredRawData[]> {
    ...
    try {
      ...
      return rawData.map((item) => {
        const typedItem = item as StoredRawData;
        return {
          createdAt: typedItem.createdAt,
          id: typedItem.id,
          importSessionId: typedItem.importSessionId,
          metadata: typedItem.metadata,
          processedAt: typedItem.processedAt,
          processingError: typedItem.processingError,
          processingStatus: typedItem.processingStatus,
          providerId: typedItem.providerId,
          rawData: typedItem.rawData,
          sourceId: typedItem.sourceId,
          sourceType: typedItem.sourceType,
        };
      });
    } catch (error) {
  ...
  async markAsProcessed(
    ...
    try {
      ...
      this.logger.info(
        `Successfully marked ${rawTransactionIds.length} items as processed for ${sourceId}`,
      );
    } catch (error) {
  ...
  async save(
    ...
    try {
      ...
      this.logger.info(
        `Successfully saved ${saved}/${rawData.length} raw data items for ${sourceId}`,
      );
      return saved;
    } catch (error) {
  ...
  async updateProcessingStatus(
    ...
    try {
      ...
      this.logger.error(
        `Failed to update processing status for ${rawTransactionId}: ${String(error)}`,
      );
      throw error;
    }
  }
