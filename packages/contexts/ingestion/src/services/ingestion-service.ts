...
import type { Database } from '../config.js';
import type { Logger } from '../pino-logger.js';
import { getLogger } from '../pino-logger.js';
import { ImporterFactory } from '../shared/importers/importer-factory.js';
import type { ImportParams, ImportResult } from '../shared/importers/interfaces.js';
import type {
  ApiClientRawData,
...
import { ProcessorFactory } from '../shared/processors/processor-factory.js';
...
import type { LoadRawDataFilters } from './raw-data-repository.js';
...
export class TransactionIngestionService {
  ...
  constructor(
    ...
    this.sessionRepository = new ImportSessionRepository();
  ...
  async importFromSource(
    ...
    try {
      importSessionId = await this.sessionRepository.create(
        sourceId,
        sourceType,
        params.providerId,
        {
  ...
  async processAndStore(
    ...
    try {
      ...
      const sessionsWithRawData = (await this.database.getImportSessionsWithRawData(sourceId)) as {
      ...
      for (const sessionData of sessionsToProcess) {
        ...
        this.logger.debug(
          `Processed ${sessionTransactions.length} transactions for session ${session.id}`,
        );
      }
      ...
      if (skippedCount > 0) {
        this.logger.info(
          `${skippedCount} items were processed but skipped (likely non-standard operation types)`,
        );
      }

      this.logger.info(
        `Processing completed for ${sourceId}: ${savedCount} processed, ${failed} failed`,
      );
