...
import type { UniversalTransaction, StoredRawData, TransactionType } from '../../types.js';
import { UniversalTransactionSchema } from '../../universal-schemas.js';
...
export abstract class BaseProcessor<TRawData> implements IProcessor {
  ...
  async process(importSession: ProcessingImportSession): Promise<UniversalTransaction[]> {
    this.logger.info(
      `Processing ${importSession.rawDataItems.length} raw data items for ${this.sourceId}`,
    );
    ...
    this.logger.info(
      `Processing completed for ${this.sourceId}: ${valid.length} valid, ${invalid.length} invalid`,
    );
