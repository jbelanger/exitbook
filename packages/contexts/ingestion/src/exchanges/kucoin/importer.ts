...
export class KucoinCsvImporter extends BaseImporter<CsvKuCoinRawData> {
  ...
  async import(params: ImportParams): Promise<ImportRunResult<CsvKuCoinRawData>> {
    this.logger.info(
      `Starting KuCoin CSV import from directories: ${params.csvDirectories?.join(', ') ?? 'none'}`,
    );
    ...
    try {
        ...
        try {
          ...
          for (const file of csvFiles) {
            ...
            switch (fileType) {
              ...
              case 'convert':
                this.logger.warn(
                  `Skipping convert orders CSV file - using account history instead: ${file}`,
                );
                break;
              case 'unknown':
      ...
      rawData.spotOrders.sort(
        (a, b) =>
          new Date(a['Filled Time(UTC)']).getTime() - new Date(b['Filled Time(UTC)']).getTime(),
      );
      rawData.deposits.sort(
        (a, b) => new Date(a['Time(UTC)']).getTime() - new Date(b['Time(UTC)']).getTime(),
      );
