...
export type ValidatedRawCoinbaseTransactionsResponse = z.infer<
  typeof RawCoinbaseTransactionsResponseSchema
>;
export type ValidatedRawCoinbaseAccountsResponse = z.infer<
  typeof RawCoinbaseAccountsResponseSchema
>;
 ...
 */
export function formatCoinbaseValidationErrors<T>(
  result: CoinbaseApiBatchValidationResult<T>,
): string {
  ...
  const additionalErrors =
    result.invalid.length > 3 ? ` and ${result.invalid.length - 3} more` : '';
