...
import { parseDecimal, createMoney } from '../../decimal-utils.js';
...
export class KrakenProcessor extends BaseProcessor<ApiClientRawData<CsvKrakenLedgerRow>> {
  ...
  private convertTradeToTransaction(
    ...
    const finalReceiveAmount = isReceiveFee
      ? String(parseDecimal(receiveAmount).minus(totalFee))
      : receiveAmount;
  ...
  private isFailedTransactionPair(
    ...
    const feesAreOpposite =
      negativeFee.gt(0) && positiveFee.lt(0) && negativeFee.plus(positiveFee).abs().lt(0.001);
  ...
  private parseLedgers(rows: CsvKrakenLedgerRow[]): UniversalTransaction[] {
    ...
    const { validWithdrawals } = this.filterFailedTransactions(
    ...
    for (const [refId, group] of tradeGroups) {
      if (group.length === 2) {
        ...
        const receive = group.find(
          (row) =>
            parseDecimal(row.amount).gt(0) && (row.type === 'receive' || row.type === 'trade'),
        );
      ...
      if (group.length === 2) {
        ...
        if (negative && positive && negative.asset !== positive.asset) {
          ...
          if (relativeDiff.lt(0.001)) {
            ...
            const migrationTransaction = this.convertTokenMigrationToTransaction(
              negative,
              positive,
            );
