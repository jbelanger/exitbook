...
function validateProviders(): void {
  ...
  for (const provider of allProviders) {
    ...
    if (result.isErr()) {
      ...
      allErrors.push(
        ...errors.map((err) => `${provider.blockchain}:${provider.name} - ${err.message}`),
      );
      allWarnings.push(
        ...warnings.map((warn) => `${provider.blockchain}:${provider.name} - ${warn.message}`),
      );
    } else {
      allWarnings.push(
        ...result.value.map((warn) => `${provider.blockchain}:${provider.name} - ${warn}`),
      );
    }
  }
