import type { BlockchainExplorersConfig, ProviderOverride } from '../../config.js';
...
export class BlockchainProviderManager {
  ...
  autoRegisterFromConfig(
    ...
    try {
      ...
      if (!this.explorerConfig) {
        logger.info(
          `No configuration file found. Using all registered providers for ${blockchain}`,
        );
      ...
      if (!blockchainConfig) {
        logger.info(
          `No configuration found for blockchain: ${blockchain}. Using all registered providers.`,
        );
        return this.autoRegisterFromRegistry(blockchain, network, preferredProvider);
      }
      ...
      if (
        ...
        (blockchainConfig.defaultEnabled === undefined ||
          Array.isArray(blockchainConfig.defaultEnabled)) &&
  ...
  private autoRegisterFromRegistry(
    ...
    try {
      ...
      if (preferredProvider) {
        const matchingProvider = registeredProviders.find(
          (provider) => provider.name === preferredProvider,
        );
        if (matchingProvider) {
      ...
      for (const providerInfo of registeredProviders) {
        try {
          ...
          if (!networkEndpoint) {
            logger.warn(
              `Provider ${providerInfo.name} does not support network ${network}. Skipping.`,
            );
            continue;
          }
  ...
  private handleOverrideConfig(
    ...
    const defaultEnabled =
      blockchainConfig.defaultEnabled || allRegisteredProviders.map((p) => p.name);
    ...
    if (preferredProvider) {
      ...
      for (const providerName of defaultEnabled) {
        if (!allRegisteredProviders.some((p) => p.name === providerName)) {
          logger.warn(
            `Default provider '${providerName}' not registered for ${blockchain}. Skipping.`,
          );
          continue;
        }
