...
export class BitcoinUtils {
  ...
  > {
    ...
    try {
      ...
      const addressType =
        xpubType === 'xpub' ? 'legacy' : xpubType === 'ypub' ? 'segwit' : 'bech32';
  ...
  static async initializeXpubWallet(
    ...
    try {
      ...
      const { addressFunction, addressType, bipStandard, hdNode } =
        await this.smartDetectAccountType(walletAddress.address, network, providerManager);
    ...
    for (let i = 0; i < allDerived.length; i++) {
      ...
      try {
        const result = await providerManager.executeWithFailover('bitcoin', {
          address,
          getCacheKey: (params) =>
            `bitcoin:address-info:${(params as { address: string }).address}`,
          type: 'getAddressInfo',
        });
