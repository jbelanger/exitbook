...
@RegisterApiClient({
  ...
  description:
    'Blockchain.com Bitcoin explorer API with transaction and balance data (no API key required)',
...
export class BlockchainComApiClient extends BaseRegistryProvider {
  ...
  private async getAddressInfo(params: { address: string }): Promise<AddressInfo> {
    ...
    try {
      const addressInfo = await this.httpClient.get<BlockchainComAddressResponse>(
        `/rawaddr/${address}?limit=0`,
      );
      ...
      this.logger.debug(
        `Successfully retrieved raw address info - Address: ${maskAddress(address)}`,
      );
  ...
  private async getRawAddressTransactions(params: {
    ...
    try {
      ...
      const addressData = await this.httpClient.get<BlockchainComAddressResponse>(
        `/rawaddr/${address}?limit=50`,
      );
