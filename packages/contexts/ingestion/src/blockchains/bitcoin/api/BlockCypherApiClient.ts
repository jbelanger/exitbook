...
import { hasStringProperty, isErrorWithMessage } from '../../../type-guards.js';
...
export class BlockCypherApiClient extends BaseRegistryProvider {
  ...
  private async fetchCompleteTransaction(txHash: string): Promise<BlockCypherTransaction> {
    ...
    const initialTx = await this.httpClient.get<BlockCypherTransaction>(
    ...
    while (transaction.next_outputs) {
      this.logger.debug(
        `Fetching next outputs for transaction ${txHash}: ${transaction.next_outputs}`,
      );
      try {
    ...
    while (transaction.next_inputs) {
      this.logger.debug(
        `Fetching next inputs for transaction ${txHash}: ${transaction.next_inputs}`,
      );
      try {
    ...
    try {
      ...
      for (let i = 0; i < uniqueTxHashes.length; i += batchSize) {
        ...
        rawTransactions.push(
          ...batchTransactions.filter((tx): tx is BlockCypherTransaction => tx !== null),
        );
