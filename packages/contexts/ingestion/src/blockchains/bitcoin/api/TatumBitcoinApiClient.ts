...
export class TatumBitcoinApiClient extends TatumApiClientBase<
...
> {
  ...
  async getRawAddressTransactions(
    ...
    try {
      ...
      this.logger.debug(
        `Retrieved raw transactions - Address: ${maskAddress(address)}, Count: ${transactions.length}`,
      );
  ...
  override async isHealthy(): Promise<boolean> {
    try {
      ...
      await this.makeRequest<TatumBitcoinBalance>(
  ...
  private async getAddressInfo(params: { address: string }): Promise<AddressInfo> {
    ...
    try {
      ...
      this.logger.debug(
        `Retrieved address info - Address: ${maskAddress(address)}, TxCount: ${txCount}`,
      );
