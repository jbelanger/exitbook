...
import type { AddressInfo, MempoolAddressInfo, MempoolTransaction } from '../types.js';
    ...
    try {
      ...
      if (since) {
        filteredTransactions = rawTransactions.filter((tx) => {
          const timestamp =
            tx.status.confirmed && tx.status.block_time ? tx.status.block_time * 1000 : Date.now();
