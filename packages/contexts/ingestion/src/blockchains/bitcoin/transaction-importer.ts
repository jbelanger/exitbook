...
export class BitcoinTransactionImporter extends BaseImporter<BitcoinTransaction> {
  ...
  async import(params: ImportParams): Promise<ImportRunResult<BitcoinTransaction>> {
    ...
    this.logger.info(
      `Starting Bitcoin transaction import for address: ${params.address.substring(0, 20)}...`,
    );
    ...
    if (wallet.derivedAddresses) {
      ...
      try {
        const rawTransactions = await this.fetchRawTransactionsForAddress(
          params.address,
          params.since,
        );

        // Add the source address context to each transaction
        const enhancedSourcedTransactions: ApiClientRawData<BitcoinTransaction>[] =
          rawTransactions.map((rawTx) => ({
    ...
    const metadata = wallet.derivedAddresses
  ...
  private async fetchRawTransactionsForAddress(
    ...
    try {
      ...
      this.logger.error(
        `Provider manager failed to fetch transactions for ${address}: ${String(error)}`,
      );
      throw error;
    }
  }
  ...
  private async fetchRawTransactionsForDerivedAddresses(
    ...
    for (const address of derivedAddresses) {
      ...
      try {
        ...
        this.logger.error(
          `Failed to fetch raw transactions for address ${address}: ${String(error)}`,
        );
      }
    }

    this.logger.info(
      `Found ${uniqueTransactions.size} unique raw transactions across all derived addresses`,
    );
