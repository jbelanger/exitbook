...
export class AvalancheTransactionProcessor extends BaseProcessor<
  ApiClientRawData<AvalancheRawTransactionData>
> {
  ...
  protected processInternal(
    ...
    this.correlationLogger.info(
      `Processing ${rawDataItems.length} Avalanche transactions with integrated correlation`,
    );
    ...
    for (const rawDataItem of rawDataItems) {
      ...
      if (!processor) {
        return Promise.resolve(
          err(`No processor found for provider: ${apiClientRawData.providerId}`),
        );
      }
      ...
      if (blockchainTransactions.length === 0) {
        this.correlationLogger.warn(
          `No transactions returned from ${apiClientRawData.providerId} processor`,
        );
        continue;
      }
    ...
    const transactionGroups = new Map<
      string,
      { txGroup: UniversalBlockchainTransaction[]; userAddress: string }
    >();
    ...
    for (const tx of universalTransactions) {
      if (!transactionGroups.has(tx.id)) {
        ...
        const userAddress =
          rawDataItems.find(
            (item) => item.rawData.rawData && universalTransactions.some((utx) => utx.id === tx.id),
    ...
    for (const [hash, { txGroup, userAddress }] of transactionGroups) {
      ...
      if (correlationResult.isErr()) {
        this.correlationLogger.error(
          `Failed to correlate group ${hash}: ${correlationResult.error}`,
        );
        continue;
      }
