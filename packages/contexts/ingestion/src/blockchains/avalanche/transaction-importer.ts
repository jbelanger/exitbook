 ...
 */
export type AvalancheRawTransactionData =
  | SnowtraceTransaction
  | SnowtraceInternalTransaction
  | SnowtraceTokenTransfer;
...
export class AvalancheTransactionImporter extends BaseImporter<AvalancheRawTransactionData> {
  ...
  async import(params: ImportParams): Promise<ImportRunResult<AvalancheRawTransactionData>> {
    ...
    const addressTransactions = await this.fetchRawTransactionsForAddress(
      params.address,
      params.since,
    );
  ...
  private async fetchRawTransactionsForAddress(
    ...
    try {
      ...
      try {
        ...
        this.logger.debug(
          `No token transactions available for ${address.substring(0, 20)}...: ${String(error)}`,
        );
      }

      this.logger.debug(
        `Fetched ${rawTransactions.length} transactions for address: ${address.substring(0, 20)}...`,
      );
