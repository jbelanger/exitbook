...
export class SnowtraceApiClient extends BaseRegistryProvider {
    ...
    while (true) {
      ...
      if (res.status !== '1') {
        ...
        throw new ServiceError(
          `Snowtrace API error: ${res.message}`,
          this.name,
  ...
  private async getRawAddressBalance(params: {
    address: string;
  }): Promise<SnowtraceBalanceResponse> {
    ...
    try {
      ...
      if (res.status !== '1') {
        throw new ServiceError(
          `Failed to fetch AVAX balance: ${res.message}`,
          this.name,
          'getRawAddressBalance',
        );
      }
      ...
      this.logger.debug(
        `Retrieved raw balance for ${maskAddress(address)}: ${String(res.result)} wei`,
      );
