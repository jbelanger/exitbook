...
export class SolanaTransactionImporter extends BaseImporter<SolanaRawTransactionData> {
  ...
  async import(params: ImportParams): Promise<ImportRunResult<SolanaRawTransactionData>> {
    ...
    this.logger.info(
      `Starting Solana transaction import for address: ${params.address.substring(0, 20)}...`,
    );
    ...
    try {
      ...
      const enhancedSourcedTransactions: ApiClientRawData<SolanaRawTransactionData>[] =
    ...
    const totalTransactionCount = allSourcedTransactions.reduce(
      (acc, tx) => acc + tx.rawData.normal.length,
      0,
    );
