...
export class SolanaRPCApiClient extends BaseRegistryProvider {
  ...
  private async getRawAddressBalance(params: {
    address: string;
  }): Promise<SolanaRPCRawBalanceData> {
    ...
    this.logger.debug(
      `Fetching raw address balance - Address: ${maskAddress(address)}, Network: ${this.network}`,
    );
  ...
  }): Promise<SolanaRPCRawTransactionData> {
    ...
    try {
      ...
      for (const sig of signatures) {
        try {
          ...
          if (
            txResponse?.result &&
            (!since || (txResponse.result.blockTime && txResponse.result.blockTime >= since))
          ) {
  ...
  private async getRawTokenBalances(params: {
    ...
    this.logger.debug(
      `Fetching raw token balances - Address: ${maskAddress(address)}, Network: ${this.network}`,
    );
