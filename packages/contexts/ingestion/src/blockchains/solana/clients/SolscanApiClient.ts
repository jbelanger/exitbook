...
export class SolscanApiClient extends BaseRegistryProvider {
  ...
  private async getRawAddressBalance(params: { address: string }): Promise<SolscanRawBalanceData> {
    ...
    try {
      const response = await this.httpClient.get<SolscanResponse<{ lamports: string }>>(
        `/account/${address}`,
      );
  ...
  private async getRawAddressTransactions(params: {
    ...
    try {
      ...
      if (!response || !response.success || !response.data) {
        this.logger.debug(
          `No raw transactions found or API error - Address: ${maskAddress(address)}`,
        );
        return { normal: [] };
      }
      ...
      const filteredTransactions = since
        ? response.data.filter((tx) => tx.blockTime * 1000 >= since)
        : response.data;
