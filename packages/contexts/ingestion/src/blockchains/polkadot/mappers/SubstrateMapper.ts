...
export class SubstrateTransactionMapper extends BaseRawDataMapper<SubscanTransfer> {
  ...
  private static convertTaostatsTransaction(
    ...
    try {
      ...
      console.warn(
        `Failed to convert Taostats transaction - Tx: ${JSON.stringify(tx)}, Error: ${String(error)}`,
      );
      return null;
    }
  }

  private static processRpcBalance(
    accountInfo: SubstrateAccountInfo,
    chainConfig: SubstrateChainConfig,
  ): Balance[] {
  ...
  private static processSubscanBalance(
    rawData: SubstrateRawData,
    chainConfig: SubstrateChainConfig,
  ): Balance[] {
  ...
  protected mapInternal(
    ...
    if (!isFromUser && !isToUser) {
      return err(
        `Transaction not relevant to user addresses: ${Array.from(relevantAddresses).join(', ')}`,
      );
    }

    // Convert single SubscanTransfer directly to UniversalBlockchainTransaction
    // Pass all relevant addresses for proper matching
    const transaction = SubstrateTransactionMapper.convertSubscanTransaction(
      rawData,
      relevantAddresses,
      chainConfig,
    );
    ...
    if (!transaction) {
      return err(
        `Failed to convert transaction for addresses: ${Array.from(relevantAddresses).join(', ')}`,
      );
    }
