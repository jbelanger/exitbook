...
export abstract class BaseSubstrateApiClient extends BaseRegistryProvider {
  ...
  private async getBalanceFromExplorer(address: string): Promise<unknown> {
    try {
      ...
      this.logger.debug(
        `Explorer balance query failed - Address: ${maskAddress(address)}, Error: ${String(error)}`,
      );
      return null;
    }
  }
  ...
  private async getBalanceFromRPC(address: string): Promise<unknown> {
    ...
    try {
      ...
      this.logger.debug(
        `RPC balance query failed - Address: ${maskAddress(address)}, Error: ${String(error)}`,
      );
      return null;
    }
  }
    ...
    try {
      ...
      if (this.rpcClient) {
        try {
          ...
          this.logger.warn(
            `RPC balance query failed, trying explorer API - Error: ${String(error)}`,
          );
        }
      }
