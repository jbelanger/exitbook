...
export class PolkadotTransactionImporter extends BaseImporter<SubscanTransfer> {
  ...
  async import(params: ImportParams): Promise<ImportRunResult<SubscanTransfer>> {
    ...
    this.logger.info(
      `Starting Polkadot transaction import for address: ${params.address.substring(0, 20)}...`,
    );

    const allSourcedTransactions: ApiClientRawData<SubscanTransfer>[] = [];

    this.logger.info(
      `Importing transactions for Polkadot address: ${params.address.substring(0, 20)}...`,
    );
    ...
    try {
      ...
      if (rawData && typeof rawData === 'object' && 'data' in rawData) {
        ...
        if (Array.isArray(substrateTxData.data)) {
          const rawTransactions: ApiClientRawData<SubscanTransfer>[] = substrateTxData.data.map(
            (transfer) => ({
    ...
    this.logger.info(
      `Polkadot transaction import completed - Total: ${allSourcedTransactions.length}`,
    );
