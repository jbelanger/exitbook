...
import { BalanceRepository } from './balance-repository.ts';
...
/**
 * Factory function to create the appropriate balance repository implementation
 * based on feature toggle configuration
 */
export function createBalanceRepository(
  sqliteDb?: SQLiteDatabase,
  dbPath?: string
): BalanceRepository | KyselyBalanceRepository {
  ...
  if (config.useKysely) {
    ...
    return new BalanceRepository(sqliteDb);
  }
}
 ...
 */
export function isKyselyBalanceRepository(
  repository: BalanceRepository | KyselyBalanceRepository
): repository is KyselyBalanceRepository {
  return repository instanceof KyselyBalanceRepository;
}

/**
 * Type guard to check if repository is using SQLite3 implementation
 */
export function isSqliteBalanceRepository(
  repository: BalanceRepository | KyselyBalanceRepository
): repository is BalanceRepository {
  return repository instanceof BalanceRepository;
}
