          ...
          try {
            ...
            for (const account of accounts) {
              ...
              if (accountCursorState?.metadata?.isComplete) {
                ...
                // Preserve the completion cursor in the result
                lastSuccessfulCursorUpdates[accountId] = accountCursorState;
                accountIndex++;
                continue;
              }
              ...
              while (true) {
                ...
                if (ledgerEntries.length === 0) {
                  // Empty account - mark as complete to prevent reprocessing
                  lastSuccessfulCursorUpdates[accountId] = {
                ...
                if (cursorUpdates[accountId]) {
                  ...
                  if (isComplete && cursorUpdates[accountId].metadata) {
                    cursorUpdates[accountId].metadata.isComplete = true;
                  }
                }
