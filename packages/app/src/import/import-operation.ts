...
const logger = getLogger('ImportOperation');
 ...
 *
 * Uses DataContext directly — orchestration, not domain logic.
 */
export class ImportOperation {
  ...
  // ---------------------------------------------------------------------------
  // Streaming import (core loop — absorbed from StreamingImportRunner)
  // ---------------------------------------------------------------------------
  ...
  private async executeStreamingImport(
    ...
    const incompleteResult = await this.db.importSessions.findLatestIncomplete(account.id);
    ...
    if (incompleteSession) {
      ...
      const createResult = await this.db.importSessions.create(account.id);
