...
@RegisterProvider({
  ...
  description:
    "Direct connection to Injective Protocol blockchain explorer with comprehensive transaction data",
  capabilities: {
    supportedOperations: [
...
export class InjectiveExplorerProvider extends BaseRegistryProvider {
  ...
  constructor() {
    ...
    this.logger.debug(
      `Initialized InjectiveExplorerProvider from registry metadata - Network: ${this.network}, BaseUrl: ${this.baseUrl}`,
    );
  }
  ...
  private async getAddressTransactions(params: {
    ...
    try {
      ...
      const data = (await this.httpClient.get(
        endpoint,
      )) as InjectiveApiResponse;

      if (!data.data || !Array.isArray(data.data)) {
        this.logger.debug(
          `No transactions found in API response - Address: ${maskAddress(address)}, HasData: ${!!data.data}`,
        );
        return [];
      }
      ...
      for (const tx of data.data) {
        try {
          ...
          this.logger.warn(
            `Failed to parse transaction - TxHash: ${tx.hash || tx.id}, Error: ${error instanceof Error ? error.message : error}`,
          );
        }
      }
  ...
  private async getRawAddressTransactions(params: {
    ...
    this.logger.debug(
      `Fetching raw address transactions - Address: ${maskAddress(address)}, Network: ${this.network}`,
    );
  ...
  private parseInjectiveTransaction(
    ...
    if (
      tx.gas_fee &&
      tx.gas_fee.amount &&
      Array.isArray(tx.gas_fee.amount) &&
      tx.gas_fee.amount.length > 0
    ) {
    ...
    let transactionType: "transfer_in" | "transfer_out" | "transfer" =
      "transfer";
    ...
    for (const message of tx.messages) {
      ...
      if (message.type === "/cosmos.bank.v1beta1.MsgSend") {
        ...
        if (message.value.amount && Array.isArray(message.value.amount) && message.value.amount.length > 0) {
          ...
          if (transferAmount) {
            value = createMoney(
              parseDecimal(transferAmount.amount)
                .div(Math.pow(10, 18))
                .toNumber(),
      ...
      // Handle IBC transfer messages
        ...
        if (message.value.token) {
          value = createMoney(
            parseDecimal(message.value.token.amount)
              .div(Math.pow(10, 18))
              .toNumber(),
      ...
      // Handle Peggy bridge deposit messages (when funds come from Ethereum)
        ...
        if (
          messageValue.ethereum_receiver === relevantAddress ||
    ...
    if (!isRelevantTransaction) {
      this.logger.debug(
        `Skipping irrelevant transaction - Hash: ${tx.hash}, From: ${maskAddress(from)}, To: ${maskAddress(to)}, RelevantAddress: ${maskAddress(relevantAddress)}, Value: ${value.amount.toNumber()}`,
      );
      return null;
    }
      ...
      gasPrice:
        ...
        tx.gas_fee.amount.length > 0
          ? (() => {
              const firstFee = tx.gas_fee.amount[0];
              if (firstFee && firstFee.amount && firstFee.denom) {
                return parseDecimal(firstFee.amount)
                  .div(tx.gas_used || 1)
                  .toNumber();
              }
              return 0;
