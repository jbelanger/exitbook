...
export class MempoolSpaceProvider extends BaseRegistryProvider {
   ...
   */
  private parseWalletTransaction(params: { tx: unknown; walletAddresses: string[] }): BlockchainTransaction {
    ...
    try {
      const timestamp =
        mempoolTx.status.confirmed && mempoolTx.status.block_time ? mempoolTx.status.block_time * 1000 : Date.now();
