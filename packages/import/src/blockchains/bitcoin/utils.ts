...
export class BitcoinUtils {
  ...
  static async initializeXpubWallet(
    ...
    try {
      ...
      const { hdNode, addressFunction, bipStandard, addressType } =
        await this.smartDetectAccountType(
  ...
  static async performAddressGapScanning(
    ...
    logger.info(
      `Performing intelligent gap scan for ${walletAddress.address.substring(0, 20)}...`,
    );
    ...
    for (let i = 0; i < allDerived.length; i++) {
      ...
      try {
        const addressInfo = (await providerManager.executeWithFailover(
          "bitcoin",
          {
            type: "getAddressInfo",
            params: { address },
        ...
        if (hasActivity) {
          ...
          logger.debug(
            `Found activity at index ${i}: ${address} (${addressInfo.txCount} transactions)`,
          );
        } else {
          // Unused address
          consecutiveUnusedCount++;
          logger.debug(
            `No activity at index ${i}, consecutive unused: ${consecutiveUnusedCount}`,
          );

          // Early exit if we've hit the gap limit
          if (consecutiveUnusedCount >= GAP_LIMIT) {
            logger.info(
              `Reached gap limit of ${GAP_LIMIT} unused addresses, stopping scan at index ${i}`,
            );
            break;
          }
        }
        ...
        consecutiveUnusedCount++;
        logger.warn(
          `Could not check activity for address ${address} - Error: ${error}`,
        );
      }
      ...
      if (lastUsedIndex > -1 && consecutiveUnusedCount >= GAP_LIMIT) {
        logger.info(
          `Gap limit of ${GAP_LIMIT} reached after last used address at index ${lastUsedIndex}.`,
        );
        break;
      }
    }
    ...
    if (lastUsedIndex === -1) {
      ...
      finalAddressCount = Math.min(
        allDerived.length,
        lastUsedIndex + GAP_LIMIT + 1,
      );
      logger.info(
        `Scan complete. Using addresses up to index ${finalAddressCount - 1}.`,
      );
    }
    ...
    logger.info(
      `Optimized address set: ${walletAddress.derivedAddresses.length} addresses (was ${allDerived.length})`,
    );
  }
  ...
  static async smartDetectAccountType(
    ...
    if (xpub.startsWith("xpub")) {
      ...
      if (firstLegacyChild.publicKey) {
        const firstLegacyAddress = legacyAddressGen(
          Buffer.from(firstLegacyChild.publicKey),
        );

        logger.debug(
          `Checking Legacy address for activity: ${firstLegacyAddress}`,
        );
      ...
      logger.info(
        "No activity found on Legacy path. Checking for Native SegWit (Ledger-style)...",
      );
      ...
      if (firstSegwitChild.publicKey) {
        const firstSegwitAddress = segwitAddressGen(
          Buffer.from(firstSegwitChild.publicKey),
        );

        logger.debug(
          `Checking Native SegWit address for activity: ${firstSegwitAddress}`,
        );
        ...
        try {
          ...
          if (hasActivity) {
            logger.info(
              "Found activity on Native SegWit path (BIP84). Proceeding.",
            );
            return {
              hdNode: segwitHdNode,
      ...
      // Fallback to Legacy
      logger.info(
        "No activity found on any path. Defaulting to BIP44 (Legacy).",
      );
      return {
        hdNode: legacyHdNode,
        addressFunction: legacyAddressGen,
  ...
  > {
    ...
    try {
      ...
      const addressGenerator = BitcoinUtils.getAddressGenerator(
        addressType,
        network,
      );
      ...
      logger.error(
        `Failed to derive addresses from xpub - Error: ${error}, Xpub: ${xpub.substring(0, 20) + "..."}`,
      );
      throw error;
    }
  }
