...
/**
 * Validate Substrate transaction data using provider-specific schemas
 */
export function validateSubstrateTransactions(
  transactions: unknown,
  providerName: 'subscan' | 'taostats' | 'rpc'
): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];

  // Validate composite raw data structure
  const result = SubstrateRawDataSchema.safeParse(transactions);
  ...
  if (result.success && result.data) {
    if (result.data.provider !== providerName) {
      warnings.push(`Expected provider '${providerName}' but got '${result.data.provider}'`);
    }
  }
