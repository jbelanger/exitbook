...
@RegisterProvider({
  ...
  description:
    "Solana blockchain explorer API with transaction and account data access",
  capabilities: {
...
export class SolscanProvider extends BaseRegistryProvider {
  ...
  async testConnection(): Promise<boolean> {
    try {
      ...
      this.logger.debug(
        `Connection test successful - HasResponse: ${!!response}`,
      );
      return response && response.success !== false;
    } catch (error) {
      this.logger.error(
  ...
  private async getAddressTransactions(params: {
    ...
    try {
      ...
      if (!response || !response.success || !response.data) {
        this.logger.debug(
          `No transactions found or API error - Address: ${maskAddress(address)}`,
        );
        return [];
      }
  ...
  private transformTransaction(
    ...
    try {
      ...
      const userAccount = tx.inputAccount?.find(
        (acc) => acc.account === userAddress,
      );

      if (!isUserSigner && !userAccount) {
        this.logger.debug(
          `Transaction not relevant to user address - TxHash: ${tx.txHash}`,
        );
        return null;
      }
