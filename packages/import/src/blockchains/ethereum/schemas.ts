...
/**
 * Validate Ethereum transaction data using provider-specific schemas
 */
export function validateEthereumTransactions(
  transactions: unknown,
  providerName: 'alchemy' | 'moralis'
): ValidationResult {
  const errors: string[] = [];
  ...
  switch (providerName) {
    case 'alchemy':
      schema = AlchemyAssetTransferArraySchema;
      break;
    case 'moralis':
      schema = MoralisTransactionArraySchema;
      break;
    default:
