...
/**
 * Base processor for exchange transactions with shared grouping and fund flow analysis.
 *
 * This class provides the infrastructure for properly grouping related ledger entries
 * (like the two sides of a swap) and creating single atomic UniversalTransaction records.
 *
 * Subclasses must:
 * 3. Implement mapStatus() to convert exchange-specific status to UniversalTransaction status
 */
export class DefaultExchangeProcessor extends BaseTransactionProcessor {
  /**
   * Main processing method that groups entries and creates atomic transactions.
   */
  protected async processInternal(
    normalizedData: unknown[],
    _sessionMetadata?: Record<string, unknown>
  ): Promise<Result<UniversalTransaction[], string>> {
    this.logger.info(`Processing ${normalizedData.length} ledger entries for ${this.sourceId}`);

    const entries = normalizedData as ExchangeLedgerEntry[];
    const entryGroups = this.groupEntriesByCorrelationId(entries);

    this.logger.debug(`Created ${entryGroups.size} entry groups for correlation on ${this.sourceId}`);
    ...
    for (const [correlationId, entryGroup] of entryGroups) {
      ...
      if (!primaryEntry) {
        ...
        processingErrors.push({ correlationId, entryCount: entryGroup.length, error: errorMsg });
        this.logger.error(
          `${errorMsg} ${correlationId} (${entryGroup.length} entries) - THIS TRANSACTION GROUP WILL BE LOST. Group types: ${entryGroup.map((e) => e.type).join(', ')}`
        );
        continue;
      }
      ...
      const universalTransaction: UniversalTransaction = {
        ...
        fees: {
          network: undefined, // Exchanges don't have network fees
          platform:
        ...
        metadata: {
          ...
          ledgerEntries: entryGroup.map((e) => e.id),
        },
      };
  ...
  /**
   * Group ledger entries by their correlation ID.
   */
  protected groupEntriesByCorrelationId(entries: ExchangeLedgerEntry[]): Map<string, ExchangeLedgerEntry[]> {
    const groups = new Map<string, ExchangeLedgerEntry[]>();

    for (const entry of entries) {
      if (!entry?.id) {
        this.logger.warn('Encountered entry without id during grouping');
        continue;
      }
   ...
   */
  protected analyzeFundFlow(entryGroup: ExchangeLedgerEntry[]): Result<ExchangeFundFlow, string> {
    ...
    const fees: { amount: string; currency: string }[] = [];
    ...
    for (const entry of entryGroup) {
      ...
      if (entry.fee && !parseDecimal(entry.fee).isZero()) {
        const feeCurrency = entry.feeCurrency || asset;
        fees.push({
          amount: entry.fee,
    ...
    const consolidatedInflows = this.consolidateMovements(inflows);
    const consolidatedOutflows = this.consolidateMovements(outflows);
    const consolidatedFees = this.consolidateFees(fees);
    ...
    return ok({
      ...
      correlationId: primaryEntry.correlationId,
  ...
  protected selectPrimaryEntry(
    entryGroup: ExchangeLedgerEntry[],
