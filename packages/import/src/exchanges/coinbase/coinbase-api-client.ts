...
export class CoinbaseAPIClient {
  ...
  constructor(credentials: CoinbaseCredentials) {
    ...
    this.logger.info(`Coinbase API client initialized - BaseUrl: ${this.baseUrl}, Sandbox: ${credentials.sandbox || false}`);
  }
  ...
  async getAccounts(params: CoinbaseAccountsParams = {}): Promise<RawCoinbaseAccount[]> {
    ...
    const response = await this.authenticatedRequest<RawCoinbaseAccountsResponse>(
  ...
  async getAccountLedger(
    ...
    this.logger.debug(`Fetching account ledger - AccountId: ${accountId}, Params: ${JSON.stringify(params)}`);
    ...
    this.logger.debug(`Retrieved ${entriesCount} ledger entries for account ${accountId} - HasNext: ${response.has_next}, Cursor: ${response.cursor ? 'present' : 'none'}`);
  ...
  async getAllAccountLedgerEntries(
    ...
    this.logger.info(`Starting paginated ledger fetch for account ${accountId}`);

    do {
      pageCount++;
      
      if (pageCount > maxPages) {
        this.logger.warn(`Reached maximum page limit (${maxPages}) for account ${accountId}, stopping pagination`);
        break;
      }
      ...
      if (response.ledger && response.ledger.length > 0) {
        ...
        this.logger.debug(`Page ${pageCount}: Retrieved ${response.ledger.length} entries - Total: ${allEntries.length}, HasNext: ${response.has_next}`);
      } else {
        this.logger.debug(`Page ${pageCount}: No entries returned, ending pagination`);
        break;
      }
    } while (cursor);

    this.logger.info(`Completed paginated ledger fetch for account ${accountId} - TotalEntries: ${allEntries.length}, Pages: ${pageCount}`);
  ...
  private async authenticatedRequest<T>(
    ...
    this.logger.debug(`Making authenticated request - Method: ${method}, Path: ${fullPath}, Timestamp: ${timestamp}`);
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('401') || error.message.includes('403')) {
          this.logger.error(`Authentication failed - Method: ${method}, Path: ${fullPath}, Error: ${error.message}. Check API credentials and permissions.`);
        } else {
