...
describe('CoinbaseAPIClient', () => {
  ...
  describe('authentication', () => {
    ...
    it('should generate correct authentication headers for GET request with query parameters', async () => {
      ...
      const message = '1640995200' + 'GET' + '/api/v3/brokerage/accounts?limit=50&cursor=test-cursor' + '';
      const expectedSignature = crypto
        .createHmac('sha256', credentials.secret)
        .update(message)
        .digest('hex');
  ...
  describe('getAccountLedger', () => {
    ...
    it('should throw error for empty account ID', async () => {
      await expect(client.getAccountLedger('')).rejects.toThrow('Account ID is required for ledger requests');
    });
  });
  ...
  describe('getAllAccountLedgerEntries', () => {
    ...
    it('should handle pagination correctly', async () => {
      ...
      expect(mockHttpClient.request.mock.calls[0][0]).toBe(`/api/v3/brokerage/accounts/${testAccountId}/ledger?limit=100`);
      
      // Verify second call had cursor
      expect(mockHttpClient.request.mock.calls[1][0]).toBe(`/api/v3/brokerage/accounts/${testAccountId}/ledger?limit=100&cursor=cursor-2`);
    });
    ...
    it('should respect custom limit parameter', async () => {
      ...
      expect(mockHttpClient.request.mock.calls[0][0]).toBe(`/api/v3/brokerage/accounts/${testAccountId}/ledger?limit=25`);
    });
  });
  ...
  describe('testConnection', () => {
    ...
    it('should return false for failed connection', async () => {
      mockHttpClient.request.mockRejectedValue(new Error('Authentication failed'));
