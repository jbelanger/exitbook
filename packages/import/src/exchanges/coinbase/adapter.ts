...
export class CoinbaseAdapter extends BaseAdapter {
  ...
  constructor(config: UniversalExchangeAdapterConfig, credentials: CoinbaseCredentials) {
    super(config);
  ...
  protected async fetchRawTransactions(params: UniversalFetchParams): Promise<RawCoinbaseTransaction[]> {
    ...
    for (const account of this.accounts) {
      ...
      try {
        const accountTransactions = await this.fetchAccountTransactions(account.id, params);
    ...
    while (hasNextPage && pageCount < maxPages) {
      ...
      if (response.data && response.data.length > 0) {
        ...
        if (params.since) {
          ...
          filteredTransactions = response.data.filter(tx => new Date(tx.created_at) >= sinceDate);
        }
        if (params.until) {
          const untilDate = new Date(params.until);
          filteredTransactions = filteredTransactions.filter(tx => new Date(tx.created_at) <= untilDate);
        }
        ...
        this.logger.debug(`Page ${pageCount}: No transactions returned, ending pagination`);
        break;
      }
    }
    ...
    for (const group of groupedTransactions) {
      try {
        ...
        if (group.length === 1) {
          ...
          this.logger.debug(
            `PROCESSING_COMBINED - Group size: ${group.length}, TXs: ${group.map(tx => tx.id).join(', ')}`
          );
  ...
  protected async transformBalances(rawAccounts: RawCoinbaseAccount[]): Promise<UniversalBalance[]> {
  ...
  private async loadAccounts(): Promise<void> {
    ...
    try {
      ...
      this.logger.info(`Loaded ${this.accounts.length} Coinbase accounts`);
    } catch (error) {
      this.logger.error(
   ...
   */
  private createTransactionFromTrackAPI(tx: RawCoinbaseTransaction): UniversalTransaction | null {
    try {
      ...
      this.logger.debug(
        `RAW TX: ${tx.id}, Type: ${tx.type}, Amount: ${tx.amount.amount} ${tx.amount.currency}, Status: ${tx.status}`
      );
      if (tx.buy)
        this.logger.debug(
          `  BUY: total=${tx.buy.total?.amount} ${tx.buy.total?.currency}, subtotal=${tx.buy.subtotal?.amount}`
        );
      if (tx.sell)
        this.logger.debug(
          `  SELL: total=${tx.sell.total?.amount} ${tx.sell.total?.currency}, subtotal=${tx.sell.subtotal?.amount}`
        );
      ...
      switch (tx.type) {
        ...
        case 'send':
          ...
          if (new Decimal(tx.amount.amount).isPositive()) {
            type = 'deposit';
            this.logger.debug(
              `SEND_CLASSIFICATION - TX: ${tx.id}, Amount: ${tx.amount.amount} ${tx.amount.currency} -> DEPOSIT (positive amount)`
            );
          } else {
            type = 'withdrawal';
            this.logger.debug(
              `SEND_CLASSIFICATION - TX: ${tx.id}, Amount: ${tx.amount.amount} ${tx.amount.currency} -> WITHDRAWAL (negative amount)`
            );
          }
          break;
        case 'deposit':
        ...
        case 'retail_simple_dust':
          ...
          break;
        default:
          this.logger.debug(`Unknown transaction type: ${tx.type}, treating as transfer`);
          type = 'transfer';
      ...
      this.logger.debug(
        `FINAL_TX - TX: ${tx.id}, Type: ${type}, Side: ${side}, Amount: ${baseTransaction.amount.amount} ${baseTransaction.amount.currency}, Price: ${baseTransaction.price?.amount} ${baseTransaction.price?.currency}`
      );
  ...
  private groupAdvancedTradeFills(
    ...
    const sortedFills = advancedTradeFills.sort(
  ...
  private areMatchingTradePair(tx1: RawCoinbaseTransaction, tx2: RawCoinbaseTransaction): boolean {
    ...
    if (tx1.native_amount && tx2.native_amount) {
      ...
      if (avgNative.greaterThan(0)) {
        ...
        if (percentDiff.lessThanOrEqualTo(0.05)) {
          // Within 5%
          return true;
        }
      }
  ...
  private extractTradeGroupIdFromTrackAPI(tx: RawCoinbaseTransaction): string | null {
    ...
    this.logger.debug(
      `GROUP_ID - TX: ${tx.id}, Type: ${tx.type}, Buy.id: ${tx.buy?.id}, Sell.id: ${tx.sell?.id}, Trade.id: ${tx.trade?.id}`
    );
  ...
  private combineTradeTransactions(transactions: RawCoinbaseTransaction[]): UniversalTransaction | null {
    ...
    this.logger.debug(`COMBINE_TRADES - Combining ${transactions.length} Track API transactions into single trade`);
    ...
    const traditionalFiatTransaction = transactions.find(tx => traditionalFiatCurrencies.includes(tx.amount.currency));
    ...
    if (traditionalFiatTransaction) {
      ...
      cryptoTransaction = transactions.find(tx => !allFiatCurrencies.includes(tx.amount.currency)) || baseTransaction;
      fiatTransaction =
        transactions.find(tx => allFiatCurrencies.includes(tx.amount.currency)) ||
        transactions.find(tx => tx !== cryptoTransaction) ||
        baseTransaction;
    }
    ...
    const quoteCurrency =
      fiatTransaction?.amount.currency ||
    ...
    this.logger.debug(
      `COMBINED_RESULT - ID: ${result.id}, Type: ${result.type}, Side: ${result.side}, Amount: ${result.amount.amount} ${result.amount.currency}, Price: ${result.price?.amount} ${result.price?.currency}, Symbol: ${result.symbol}`
    );
  ...
  private extractTradeAmount(
    ...
    this.logger.debug(
      `EXTRACT_AMOUNT - TX: ${tx.id}, Type: ${type}, Raw Amount: ${tx.amount.amount} ${tx.amount.currency}`
    );
