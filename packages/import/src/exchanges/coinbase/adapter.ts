...
export class CoinbaseAdapter extends BaseAdapter {
  ...
  constructor(
    ...
    this.logger.info(`Initialized Coinbase Direct adapter - Exchange: ${config.id}, Sandbox: ${credentials.sandbox || false}`);
  }
  ...
  protected async fetchRawTransactions(params: UniversalFetchParams): Promise<RawCoinbaseLedgerEntry[]> {
    const requestedTypes = params.transactionTypes || ['trade', 'deposit', 'withdrawal'];
    ...
    for (const account of this.accounts) {
      try {
        this.logger.debug(`Fetching ledger for account ${account.uuid} (${account.currency})`);
        ...
        this.logger.debug(`Fetched ${entries.length} ledger entries for account ${account.uuid} (${account.currency})`);
      } catch (accountError) {
        this.logger.warn(`Failed to fetch ledger for account ${account.uuid} - Error: ${accountError instanceof Error ? accountError.message : 'Unknown error'}`);
        // Continue with other accounts instead of failing completely
      }
    }

    this.logger.info(`Fetched ${allEntries.length} total ledger entries from ${this.accounts.length} accounts`);
    return allEntries;
  }
  ...
  protected async transformTransactions(
    ...
    const requestedTypes = params.transactionTypes || ['trade', 'deposit', 'withdrawal'];
    this.logger.info(`Transforming ${rawEntries.length} raw Coinbase ledger entries for types: ${requestedTypes.join(', ')}`);
    ...
    for (const [orderId, entries] of tradeGroups.entries()) {
      try {
        ...
        this.logger.warn(`Failed to create trade from group ${orderId} - Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    ...
    const nonTradeEntries = rawEntries.filter(entry => !this.isTradeRelatedEntry(entry));
    for (const entry of nonTradeEntries) {
      try {
        ...
        this.logger.warn(`Failed to transform entry ${entry.id} - Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }

    this.logger.info(`Transformed ${rawEntries.length} ledger entries into ${transactions.length} transactions - TradeGroups: ${tradeGroups.size}, NonTrade: ${nonTradeEntries.length}`);
    return transactions;
  }
  ...
  protected async transformBalances(
    ...
    for (const account of rawAccounts) {
      try {
        const availableAmount = new Decimal(account.available_balance?.value || '0');
        ...
        this.logger.warn(`Failed to transform balance for account ${account.uuid} - Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }

    this.logger.info(`Transformed ${rawAccounts.length} accounts into ${balances.length} non-zero balances`);
    return balances;
  }
  ...
  private async loadAccounts(): Promise<void> {
    ...
    try {
      ...
      this.logger.info(`Loaded ${this.accounts.length} active Coinbase accounts`);
    } catch (error) {
   ...
   */
  private groupTradeEntries(entries: RawCoinbaseLedgerEntry[]): Map<string, RawCoinbaseLedgerEntry[]> {
   ...
   */
  private createTradeFromEntries(orderId: string, entries: RawCoinbaseLedgerEntry[]): UniversalTransaction | null {
    ...
    const creditEntries = entries.filter(entry => entry.direction === 'CREDIT');
    ...
    if (side === 'buy') {
      ...
      baseAmount = creditEntries.reduce((sum, entry) => sum.plus(new Decimal(entry.amount.value).abs()), new Decimal(0));
      quoteAmount = debitEntries.reduce((sum, entry) => sum.plus(new Decimal(entry.amount.value).abs()), new Decimal(0));
    } else {
      ...
      baseAmount = debitEntries.reduce((sum, entry) => sum.plus(new Decimal(entry.amount.value).abs()), new Decimal(0));
      quoteAmount = creditEntries.reduce((sum, entry) => sum.plus(new Decimal(entry.amount.value).abs()), new Decimal(0));
    }

    // Calculate total fees (deduplicated by order ID)
    const totalFee = this.calculateTotalFees(entries, orderId);
    const feeCurrency = entries.find(entry => entry.details.fee)?.details.fee?.currency;
    ...
    return {
      ...
      metadata: {
        orderId,
        entries: entries.map(entry => ({ id: entry.id, type: entry.type, direction: entry.direction })),
        adapterType: 'native'
      }
    };
  }
   ...
   */
  private createTransactionFromEntry(entry: RawCoinbaseLedgerEntry): UniversalTransaction | null {
    ...
    switch (entry.type) {
      ...
      case 'MATCH':
        ...
        break;
      default:
        this.logger.debug(`Unknown ledger entry type: ${entry.type}, treating as transfer`);
        type = 'transfer';
    }
   ...
   */
  private calculateTotalFees(entries: RawCoinbaseLedgerEntry[], orderId: string): Decimal {
