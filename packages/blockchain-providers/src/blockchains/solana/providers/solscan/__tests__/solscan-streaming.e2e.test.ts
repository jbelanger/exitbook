...
describe.sequential('SolscanApiClient Streaming E2E', () => {
  ...
  /**
   * LIMITATION: Solscan V2 API endpoints (used for streaming) require a Pro API Key.
   * The standard/free key often returns 401 Unauthorized for these endpoints.
   * These tests are skipped by default to prevent CI failures.
   * To run them, ensure you have a valid SOLSCAN_API_KEY in your environment.
   */
  describe('streamAddressTransactions', () => {
    it.skip('should stream transactions with cursor management', async () => {
      ...
      for await (const result of stream) {
        ...
        if (batch.cursor.primary.type === 'pageToken') {
          expect(batch.cursor.primary.providerName).toBe('solscan');
          expect(typeof batch.cursor.primary.value).toBe('string');
          // Should be parsable as integer (offset)
          expect(!isNaN(parseInt(batch.cursor.primary.value, 10))).toBe(true);
        }
        ...
        for (const txData of batch.data) {
          ...
          expect(tx.providerName).toBe('solscan');
    ...
    it.skip('should resume streaming from cursor', async () => {
      ...
      // Note: With offset pagination, we expect the next page of results
      ...
      // Overlap should be minimal (ideally 0, but live data might shift slightly)
      const overlap = Array.from(firstIds).filter((id) => secondIds.has(id)).length;
      expect(overlap).toBeLessThan(firstBatch.data.length / 2);
    }, 60000);
    ...
    it.skip('should handle empty results gracefully', async () => {
      ...
      // Solscan might return success=false for invalid address or empty list
