...
function buildAddressInfo(overrides?: Partial<MempoolAddressInfo>): MempoolAddressInfo {
  return {
...
function buildTransaction(overrides?: Partial<MempoolTransaction>): MempoolTransaction {
  return {
    fee: 5000,
    locktime: 0,
    size: 250,
...
describe('MempoolSpaceApiClient', () => {
  const providerRegistry = createProviderRegistry();
  let client: MempoolSpaceApiClient;
  ...
  beforeEach(() => {
    ...
    client = new MempoolSpaceApiClient(config);
  ...
  describe('metadata', () => {
    it('should have correct provider identity', () => {
      expect(client).toBeInstanceOf(MempoolSpaceApiClient);
      expect(client.blockchain).toBe('bitcoin');
      expect(client.name).toBe('mempool.space');
    });

    it('should not require API key', () => {
      expect(mempoolSpaceMetadata.requiresApiKey).toBe(false);
    });
  ...
  describe('executeStreaming', () => {
    ...
    it('should stream transactions with txid-based pagination', async () => {
      ...
      expect(transactions[0]!.providerName).toBe('mempool.space');
      expect(transactions[0]!.status).toBe('success');

      // Mempool.space uses /txs/chain endpoint (not /txs like blockstream)
      expect(mockGet).toHaveBeenCalledWith(`/address/${TEST_ADDRESS}/txs/chain`, expect.anything());
    });
