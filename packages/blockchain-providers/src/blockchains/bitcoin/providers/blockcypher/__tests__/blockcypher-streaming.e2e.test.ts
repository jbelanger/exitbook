...
describe('BlockCypherApiClient Streaming E2E', () => {
  const config = ProviderRegistry.createDefaultConfig('bitcoin', 'blockcypher');
  const provider = new BlockCypherApiClient(config);
  ...
  describe('streamAddressTransactions via executeStreaming', () => {
    it('should stream transactions in batches with cursor state', async () => {
      ...
      let batchCount = 0;
      const maxBatches = 1; // Only fetch 1 batch to minimize API usage (rate limit is very low)
      ...
      for await (const result of provider.executeStreaming<BitcoinTransaction>(operation)) {
        ...
        if (batch.data.length > 0) {
          ...
          expect(firstTx.normalized.providerName).toBe('blockcypher');
    ...
    it('should extract cursors with blockNumber and timestamp', async () => {
      ...
      for await (const result of provider.executeStreaming<BitcoinTransaction>(operation)) {
        ...
        // Primary cursor should be pageToken for BlockCypher
        expect(cursor.primary.type).toBe('pageToken');
        if (cursor.primary.type === 'pageToken') {
          expect(cursor.primary.providerName).toBe('blockcypher');
          expect(typeof cursor.primary.value).toBe('string');
          // For block-based pagination, the value should be a numeric string
  ...
  describe('Cursor extraction and replay window', () => {
    ...
    it('should apply replay window to blockNumber cursor', () => {
      ...
      if (adjustedCursor.type === 'blockNumber') {
        // Replay window is 6 blocks for BlockCypher
