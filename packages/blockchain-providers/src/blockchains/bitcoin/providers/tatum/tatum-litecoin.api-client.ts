...
import {
  TatumLitecoinBalanceSchema,
  TatumLitecoinTransactionSchema,
  type TatumLitecoinTransaction,
  type TatumLitecoinBalance,
} from './tatum-litecoin.schemas.js';
...
export class TatumLitecoinApiClient extends BaseApiClient {
  ...
  async hasAddressTransactions(address: string): Promise<Result<boolean, Error>> {
    ...
    const txResult = await this.makeRequest<TatumLitecoinTransaction[]>(
  ...
  async getAddressBalances(address: string): Promise<Result<RawBalanceData, Error>> {
    ...
    const balanceResult = await this.makeRequest<TatumLitecoinBalance>(
