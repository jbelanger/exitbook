...
describe('TatumLitecoinApiClient Streaming E2E', () => {
  ...
  // Empty address for completion tests (valid bech32 format but never used)
  const emptyAddress = 'ltc1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq2x6jls';
      ...
      async () => {
        ...
        for await (const result of provider.executeStreaming<BitcoinTransaction>(operation)) {
          ...
          if (batch.data.length > 0) {
            ...
            expect(firstTx.normalized.currency).toBe('LTC');
  ...
  describe('Error handling', () => {
    ...
    it.skipIf(!process.env['TATUM_API_KEY'] || process.env['TATUM_API_KEY'] === 'YourApiKeyToken')(
      ...
      async () => {
        const invalidAddress = 'invalid-litecoin-address';
  ...
  describe('Litecoin-specific streaming', () => {
    it.skipIf(!process.env['TATUM_API_KEY'] || process.env['TATUM_API_KEY'] === 'YourApiKeyToken')(
      'should stream Litecoin transactions with UTXO structure',
      async () => {
        const operation = {
          type: 'getAddressTransactions' as const,
          address: testAddress,
        ...
        for await (const result of provider.executeStreaming<BitcoinTransaction>(operation)) {
          ...
          if (batch.data.length > 0) {
            ...
            // Litecoin transactions have inputs and outputs (UTXO model)
            ...
            // Litecoin currency
            expect(firstTx.currency).toBe('LTC');
          }
        ...
        for await (const result of provider.executeStreaming<BitcoinTransaction>(operation)) {
          ...
          if (batch.data.length > 0) {
            ...
            if (firstTx.feeAmount) {
              ...
              expect(firstTx.feeCurrency).toBe('LTC');
            }
          }
          break; // Only need first batch
        }
