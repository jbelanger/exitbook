...
describe('TatumBCashApiClient Streaming E2E', () => {
  ...
  // Empty address for completion tests (newly generated address, never used on-chain)
  const emptyAddress = 'qp0lllllllllllllllllllllllllllllllu9xdvfezs';
      ...
      async () => {
        ...
        for await (const result of provider.executeStreaming<BitcoinTransaction>(operation)) {
          ...
          if (batch.data.length > 0) {
            ...
            expect(firstTx.normalized.currency).toBe('BCH');
        ...
        for await (const result of provider.executeStreaming<BitcoinTransaction>(operation)) {
          ...
          if (batch.cursor.metadata?.isComplete) {
            ...
            if (cursor.primary.type === 'pageToken') {
              ...
              // For skip-based pagination, the value should be a numeric string
  ...
  describe('Error handling', () => {
    ...
    it.skipIf(!process.env['TATUM_API_KEY'] || process.env['TATUM_API_KEY'] === 'YourApiKeyToken')(
      ...
      async () => {
        const invalidAddress = 'invalid-bitcoin-cash-address';
  ...
  describe('Bitcoin Cash-specific streaming', () => {
    it.skipIf(!process.env['TATUM_API_KEY'] || process.env['TATUM_API_KEY'] === 'YourApiKeyToken')(
      'should stream Bitcoin Cash transactions with UTXO structure',
      async () => {
        const operation = {
          type: 'getAddressTransactions' as const,
          address: testAddress,
        ...
        for await (const result of provider.executeStreaming<BitcoinTransaction>(operation)) {
          ...
          if (batch.data.length > 0) {
            ...
            // Bitcoin Cash transactions have inputs and outputs (UTXO model)
            ...
            expect(firstTx.currency).toBe('BCH');
          }
        ...
        for await (const result of provider.executeStreaming<BitcoinTransaction>(operation)) {
          ...
          if (batch.data.length > 0) {
            ...
            if (firstTx.feeAmount) {
              ...
              expect(firstTx.feeCurrency).toBe('BCH');
            }
          }
          break; // Only need first batch
        }
    ...
    it.skipIf(!process.env['TATUM_API_KEY'] || process.env['TATUM_API_KEY'] === 'YourApiKeyToken')(
      'should handle both prefixed and unprefixed addresses',
      async () => {
        // Test with prefixed address
        const prefixedAddress = 'bitcoincash:qqqmuwfhm5arf9vlujftyxddngqfm0ckeuhdzmedl2';
