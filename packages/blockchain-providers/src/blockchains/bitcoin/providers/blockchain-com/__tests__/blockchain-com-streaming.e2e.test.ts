...
describe('BlockchainComApiClient Streaming E2E', () => {
  const config = ProviderRegistry.createDefaultConfig('bitcoin', 'blockchain.com');
  const provider = new BlockchainComApiClient(config);
  ...
  describe('streamAddressTransactions via executeStreaming', () => {
    it('should stream transactions in batches with cursor state', async () => {
      ...
      for await (const result of provider.executeStreaming<BitcoinTransaction>(operation)) {
        ...
        if (batch.data.length > 0) {
          ...
          expect(firstTx.normalized.providerName).toBe('blockchain.com');
        ...
        expect(batch.cursor.metadata?.providerName).toBe('blockchain.com');
    ...
    it('should extract cursors with blockNumber and timestamp', async () => {
      ...
      for await (const result of provider.executeStreaming<BitcoinTransaction>(operation)) {
        ...
        // Primary cursor should be pageToken for Blockchain.com
        expect(cursor.primary.type).toBe('pageToken');
        if (cursor.primary.type === 'pageToken') {
          expect(cursor.primary.providerName).toBe('blockchain.com');
