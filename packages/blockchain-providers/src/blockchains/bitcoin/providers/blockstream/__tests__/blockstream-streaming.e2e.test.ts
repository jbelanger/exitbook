...
describe('BlockstreamApiClient Streaming E2E', () => {
  const config = providerRegistry.createDefaultConfig('bitcoin', 'blockstream.info');
  const provider = new BlockstreamApiClient(config);
  ...
  describe('streamAddressTransactions via executeStreaming', () => {
    it('should stream transactions in batches with cursor state', async () => {
      ...
      for await (const result of provider.executeStreaming<BitcoinTransaction>(operation)) {
        ...
        if (batch.data.length > 0) {
          ...
          expect(firstTx.normalized.providerName).toBe('blockstream.info');
    ...
    it('should extract cursors with txHash, blockNumber and timestamp', async () => {
      ...
      for await (const result of provider.executeStreaming<BitcoinTransaction>(operation)) {
        ...
        // Primary cursor can be either pageToken (when resuming with txHash)
        // or one of the alternatives (blockNumber/timestamp when starting fresh)
        expect(cursor.primary).toBeDefined();
        if (cursor.primary.type === 'pageToken') {
          expect(cursor.primary.providerName).toBe('blockstream.info');
          expect(typeof cursor.primary.value).toBe('string');
          expect(cursor.primary.value.length).toBeGreaterThan(0);
        } else {
          // When starting fresh, primary might be blockNumber or timestamp
          expect(['blockNumber', 'timestamp']).toContain(cursor.primary.type);
        }
    ...
    it('should resume from cursor state (txHash)', async () => {
      const operation = {
        type: 'getAddressTransactions' as const,
        address: testAddress,
      };
  ...
  describe('Cursor extraction and replay window', () => {
    it('should extract txHash, blockNumber and timestamp cursors from transactions', async () => {
      ...
      for await (const result of provider.executeStreaming<BitcoinTransaction>(operation)) {
        ...
        if (txHashCursor && txHashCursor.type === 'txHash') {
          expect(typeof txHashCursor.value).toBe('string');
          expect(txHashCursor.value).toBe(confirmedTx.normalized.id);
        }
  ...
  describe('Deduplication behavior', () => {
    it('should deduplicate transactions across batches during replay', async () => {
      ...
      const crossProviderCursor: CursorState = {
        ...
        metadata: {
          providerName: 'mempool.space', // Different provider to trigger replay
          updatedAt: Date.now(),
          isComplete: false,
        },
      };
  ...
  describe('Bitcoin-specific streaming', () => {
    it('should stream Bitcoin transactions with UTXO structure', async () => {
      const operation = {
        type: 'getAddressTransactions' as const,
        address: testAddress,
      };
