  ...
  start(): void {
    ...
    this.timer = setInterval(() => {
      if (this.healthCheckInProgress) {
        logger.warn('Skipping health check tick because previous cycle is still running');
        return;
      }
  ...
  private async performHealthChecks(): Promise<void> {
    ...
    for (const result of results) {
      if (result.status === 'rejected') {
        logger.error(`Health check task failed: ${getErrorMessage(result.reason)}`);
      }
    }
  }

  private async checkProviderHealth(blockchain: string, provider: IBlockchainProvider): Promise<void> {
    try {
