...
import {
  canSafelyConvertToNumber,
  safeDecimalToNumber,
  safeMoneyToNumber,
  createMoney,
  precisionBalanceToLegacy,
  legacyBalanceToPrecision,
  precisionUniversalBalanceToLegacy,
  legacyUniversalBalanceToPrecision,
} from "../decimal-utils.js";
import type {
  PrecisionBlockchainBalance,
  BlockchainBalance,
  PrecisionUniversalBalance,
  UniversalBalance,
} from "@crypto/core";
...
describe("Precision Validation", () => {
  describe("canSafelyConvertToNumber", () => {
    ...
    it("should handle edge cases", () => {
      ...
      expect(
        canSafelyConvertToNumber(new Decimal(Number.MAX_SAFE_INTEGER)),
      ).toBe(true);
    });
  });
  ...
  describe("safeDecimalToNumber", () => {
    ...
    it("should throw error for unsafe values by default", () => {
      const largeValue = new Decimal(Number.MAX_SAFE_INTEGER).plus(1);
      expect(() => safeDecimalToNumber(largeValue)).toThrow(
    ...
    it("should call warning callback for unsafe values", () => {
      ...
      expect(warningCallback).toHaveBeenCalledWith(
        expect.stringContaining("Precision loss detected"),
      );
    });
  });
  ...
  describe("safeMoneyToNumber", () => {
    ...
    it("should handle undefined/null inputs", () => {
      expect(safeMoneyToNumber(undefined)).toBe(0);
      expect(safeMoneyToNumber(null as any)).toBe(0);
    });
  ...
  describe("Balance Conversion Functions", () => {
    describe("precisionBalanceToLegacy", () => {
      it("should convert precision balance to legacy format safely", () => {
        const precisionBalance: PrecisionBlockchainBalance = {
          currency: "BTC",
          balance: new Decimal("1.5"),
          used: new Decimal("0.5"),
          total: new Decimal("2.0"),
        };

        const legacy = precisionBalanceToLegacy(precisionBalance);
      ...
      it("should warn about precision loss", () => {
        ...
        const precisionBalance: PrecisionBlockchainBalance = {
          currency: "BTC",
          balance: new Decimal("1.123456789012345678901234567890"),
          used: new Decimal("0"),
          total: new Decimal("1.123456789012345678901234567890"),
        };

        precisionBalanceToLegacy(precisionBalance, {
          allowPrecisionLoss: true,
          warningCallback,
        });
    ...
    describe("legacyBalanceToPrecision", () => {
      it("should convert legacy balance to precision format", () => {
        const legacyBalance: BlockchainBalance = {
          currency: "ETH",
          balance: 1.5,
          used: 0.5,
          total: 2.0,
        ...
        const precision = legacyBalanceToPrecision(legacyBalance);
        expect(precision.currency).toBe("ETH");
        expect(precision.balance.toNumber()).toBe(1.5);
        expect(precision.used.toNumber()).toBe(0.5);
        expect(precision.total.toNumber()).toBe(2.0);
      });
    });

    describe("Universal Balance Conversions", () => {
      it("should convert precision universal balance to legacy", () => {
        const precisionBalance: PrecisionUniversalBalance = {
          currency: "SOL",
          total: new Decimal("10.0"),
          free: new Decimal("8.0"),
          used: new Decimal("2.0"),
        };

        const legacy = precisionUniversalBalanceToLegacy(precisionBalance);
        expect(legacy).toEqual({
          currency: "SOL",
          total: 10.0,
          free: 8.0,
      ...
      it("should convert legacy universal balance to precision", () => {
        const legacyBalance: UniversalBalance = {
          currency: "AVAX",
          total: 15.0,
          free: 12.0,
          used: 3.0,
        ...
        const precision = legacyUniversalBalanceToPrecision(legacyBalance);
        expect(precision.currency).toBe("AVAX");
        expect(precision.total.toNumber()).toBe(15.0);
        expect(precision.free.toNumber()).toBe(12.0);
        expect(precision.used.toNumber()).toBe(3.0);
      });
    });
  });
  ...
  describe("High-precision scenarios", () => {
    ...
    it("should detect precision loss in wei calculations", () => {
      ...
      expect(() => safeDecimalToNumber(weiAmount)).toThrow(
        /Precision loss detected/,
      );
    });
    ...
    it("should handle high-precision tokens", () => {
      ...
      if (!canConvert) {
        expect(() => safeDecimalToNumber(preciseAmount)).toThrow(
          /Precision loss detected/,
        );
      }
    });
