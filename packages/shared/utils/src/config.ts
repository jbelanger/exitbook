...
export class ConfigUtils {
  ...
  static loadExplorerConfig(configPath?: string): BlockchainExplorersConfig {
    ...
    try {
      ...
      throw new Error(`Failed to load blockchain explorer configuration from ${finalPath}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async loadExchangeConfig(configPath?: string): Promise<ExchangeConfiguration> {
    ...
    try {
      ...
      throw new Error(`Failed to load exchange configuration from ${finalPath}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
