...
export class ConfigUtils {
   ...
   */
  static async loadExchangeConfig(
    configPath?: string,
   ...
   */
  static resolveEnvironmentVariables(
    ...
    for (const [key, value] of Object.entries(resolved)) {
      if (typeof value === "string" && value.startsWith("env:")) {
        ...
        if (!envValue) {
          logger.warn(
            `Environment variable ${envVarName} not found for credential ${key}`,
          );
...
export const resolveEnvironmentVariables =
  ConfigUtils.resolveEnvironmentVariables;
