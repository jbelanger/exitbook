...
export class SolanaProcessor extends BaseTransactionProcessor<SolanaTransaction> {
  constructor(providerRuntime: IBlockchainProviderRuntime, scamDetector?: ScamDetector) {
    super('solana', providerRuntime, scamDetector);
  }
  ...
  protected async transformNormalizedData(
    ...
    for (const normalizedTx of normalizedData) {
      try {
        ...
        this.logger.debug(
          `Successfully processed transaction ${normalizedTx.id} - Category: ${classification.operation.category}, Type: ${classification.operation.type}, Amount: ${fundFlow.primary.amount} ${fundFlow.primary.asset}`
        );
      } catch (error) {
