 ...
 */
export function filterCsvByField<T, K extends keyof T>(rows: T[], field: K, value?: T[K]): T[] {
 ...
 * @returns Filtered array
 */
export function filterCsvByFields<T>(rows: T[], filters: Partial<Record<keyof T, unknown>>): T[] {
 ...
 * @returns Filtered array
 */
export function filterCsvByTimestamp<T extends { timestamp: number }>(rows: T[], since?: number, until?: number): T[] {
  return rows.filter((row) => {
 ...
 * @returns Filtered array
 */
export function filterCsvByUid<T extends { UID: string }>(rows: T[], uid?: string): T[] {
  return filterCsvByField(rows, 'UID', uid);
}
 ...
 */
export function groupCsvByField<T, K extends keyof T>(rows: T[], field: K): Map<T[K], T[]> {
