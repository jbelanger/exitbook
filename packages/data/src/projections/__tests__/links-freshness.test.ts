...
describe('buildLinksFreshnessPorts', () => {
  ...
  it('returns fresh when no transactions exist', async () => {
    const freshness = buildLinksFreshnessPorts(ctx, 1);
  ...
  it('returns stale when projection state is explicitly stale', async () => {
    ...
    assertOk(await repo.markStale('links', 'upstream-rebuild', buildProfileProjectionScopeKey(1)));
  ...
  it('returns failed status from projection state', async () => {
    const repo = new ProjectionStateRepository(db);
    assertOk(await repo.markFailed('links', buildProfileProjectionScopeKey(1)));
  ...
  it('returns fresh when projection state is fresh even with zero links', async () => {
    ...
    assertOk(await repo.markFresh('links', null, buildProfileProjectionScopeKey(1)));
