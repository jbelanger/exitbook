...
describe('generateDeterministicTransactionHash', () => {
  it('should generate same hash for identical transactions', () => {
    const transaction: Omit<UniversalTransactionData, 'id' | 'accountId'> = {
      ...
      movements: {
        inflows: [
          { assetId: 'test:btc', assetSymbol: 'BTC', grossAmount: new Decimal('0.5'), netAmount: new Decimal('0.5') },
        ],
        outflows: [
          {
            assetId: 'test:usdt',
  ...
  it('should generate different hashes for different amounts', () => {
    ...
    const tx2: Omit<UniversalTransactionData, 'id' | 'accountId'> = {
      ...
      movements: {
        inflows: [
          { assetId: 'test:btc', assetSymbol: 'BTC', grossAmount: new Decimal('0.6'), netAmount: new Decimal('0.6') },
        ],
        outflows: [],
      },
    };
  ...
  it('should generate same hash regardless of movement order', () => {
    const tx1: Omit<UniversalTransactionData, 'id' | 'accountId'> = {
      ...
      movements: {
        inflows: [
          { assetId: 'test:btc', assetSymbol: 'BTC', grossAmount: new Decimal('0.5'), netAmount: new Decimal('0.5') },
          { assetId: 'test:eth', assetSymbol: 'ETH', grossAmount: new Decimal('2'), netAmount: new Decimal('2') },
        ],
        outflows: [],
      },
      fees: [],
  ...
  it('should use netAmount when available, fallback to grossAmount', () => {
    const tx1: Omit<UniversalTransactionData, 'id' | 'accountId'> = {
      ...
      movements: {
        inflows: [],
        outflows: [
          { assetId: 'test:btc', assetSymbol: 'BTC', grossAmount: new Decimal('1.0'), netAmount: new Decimal('0.999') },
        ],
      },
      fees: [],
    ...
    const tx2: Omit<UniversalTransactionData, 'id' | 'accountId'> = {
      ...
      movements: {
        inflows: [],
        outflows: [{ assetId: 'test:btc', assetSymbol: 'BTC', grossAmount: new Decimal('1.0') }], // No netAmount
      },
    };
