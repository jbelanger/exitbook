...
describe('MempoolSpaceProvider', () => {
  ...
  describe('Address Transactions', () => {
    ...
    const mockAddressInfo = {
      address: testAddress,
      chain_stats: { tx_count: 2, funded_txo_sum: 200000000, spent_txo_sum: 100000000 },
  ...
  describe('Transaction Transformation', () => {
    it('should correctly classify deposit transactions', async () => {
      ...
      const depositTransaction = {
        txid: 'deposit_tx_id',
        vin: [{ prevout: { scriptpubkey_address: 'other_address', value: 100000000 } }],
        vout: [{ scriptpubkey_address: testAddress, value: 99000000 }],
        fee: 1000000,
        status: { confirmed: true, block_height: 750000, block_hash: 'hash', block_time: 1640995200 },
        version: 2,
        locktime: 0,
        size: 225,
        weight: 900
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          chain_stats: { tx_count: 1, funded_txo_sum: 99000000, spent_txo_sum: 0 },
          mempool_stats: { tx_count: 0, funded_txo_sum: 0, spent_txo_sum: 0 }
        })
      } as Response);
    ...
    it('should correctly classify withdrawal transactions', async () => {
      ...
      const withdrawalTransaction = {
        ...
        vin: [{ prevout: { scriptpubkey_address: testAddress, value: 100000000 } }],
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          chain_stats: { tx_count: 1, funded_txo_sum: 100000000, spent_txo_sum: 100000000 },
          mempool_stats: { tx_count: 0, funded_txo_sum: 0, spent_txo_sum: 0 }
        })
      } as Response);
